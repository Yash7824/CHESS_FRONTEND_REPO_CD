...
export class HomeComponent {
  ...
  movePiece(fromRow: number, fromCol: number, toRow: number, toCol: number): void {
    ...
    switch(piece){
      case 'P': this.whitePawnMovement(fromRow, fromCol, toRow, toCol); break;
      case 'R': this.whiteRookMovement(fromRow, fromCol, toRow, toCol); break;
      case 'N': this.whiteKnightMovement(fromRow, fromCol, toRow, toCol); break;
      case 'B': this.whiteBishopMovement(fromRow, fromCol, toRow, toCol); break;
      case 'Q': this.whiteQueenMovement(fromRow, fromCol, toRow, toCol); break;
      case 'K': this.whiteKingMovement(fromRow, fromCol, toRow, toCol); break;
      case 'p': this.blackPawnMovement(fromRow, fromCol, toRow, toCol); break;
      case 'r': this.blackRookMovement(fromRow, fromCol, toRow, toCol); break;
      case 'n': this.blackKnightMovement(fromRow, fromCol, toRow, toCol); break;
      case 'b': this.blackBishopMovement(fromRow, fromCol, toRow, toCol); break;
      case 'q': this.blackQueenMovement(fromRow, fromCol, toRow, toCol); break;
      case 'k': this.blackKingMovement(fromRow, fromCol, toRow, toCol); break;
  ...
  IsWhitePiece(row: number, col: number): boolean{
    ...
    if(piece == 'R' || piece == 'N' || piece == 'B' || piece == 'Q' || piece == 'K' || piece == 'P') return true;
    return false;
  }
  ...
  resetToPreviousPosition(fromRow: number, fromCol: number, toRow: number, toCol: number){
    const piece = this.chess_Board[fromRow][fromCol];
    this.chess_Board[toRow][toCol] = '';
    this.chess_Board[fromRow][fromCol] = piece;
  }

  IsInvalidMove(piece: string, fromRow: number, fromCol: number, toRow: number, toCol: number): boolean{
    if(fromRow == toRow && fromCol == toCol) return true;
      
    switch(piece){
      case 'P': {
        //moving diagonally
        if(toRow == fromRow - 1 && (toCol == fromCol - 1 || toCol == fromCol + 1)) return true; break;
      }
      ...
      (toCol == fromCol+2 && (toRow == fromRow+1 || toRow == fromRow-1)))) return true; break;
      }
  ...
  whitePawnMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){
    // debugger;
    const piece = this.chess_Board[fromRow][fromCol];

    // Attacking Black Pieces 
    if((toRow == fromRow-1 && (toCol == fromCol-1 || toCol == fromCol+1)) && this.IsBlackPiece(toRow, toCol)){
  ...
  whiteRookMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){
    const piece = this.chess_Board[fromRow][fromCol];
    debugger;

    //Attacking Black Pieces
    if(this.IsBlackPiece(toRow, toCol) && !this.IsInvalidMove(piece, fromRow, fromCol, toRow, toCol)){
      this.chess_Board[toRow][toCol] = piece;
      this.chess_Board[fromRow][fromCol] = '';
    }
    // If the tile is empty
  ...
  whiteKnightMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){
    const piece = this.chess_Board[fromRow][fromCol];
    ...
    (toRow == fromRow+2 && (toCol == fromCol+1 || toCol == fromCol-1)) ||
    (toCol == fromCol-2 && (toRow == fromRow+1 || toRow == fromRow-1)) ||
    (toCol == fromCol+2 && (toRow == fromRow+1 || toRow == fromRow-1))) && this.IsBlackPiece(toRow, toCol)){
      this.chess_Board[toRow][toCol] = piece;
      this.chess_Board[fromRow][fromCol] = '';
    }
  ...
  whiteBishopMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){

  }

  whiteQueenMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){

  }

  whiteKingMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){

  }

  // Black Pieces 
  blackPawnMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){
    const piece = this.chess_Board[fromRow][fromCol];

    //Attacking White Pieces
    if((toRow == fromRow+1 && (toCol == fromCol-1 || toCol == fromCol+1)) && this.IsWhitePiece(toRow, toCol)){
  ...
  blackRookMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){
    
  }

  blackKnightMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){
    const piece = this.chess_Board[fromRow][fromCol];
    ...
    (toRow == fromRow+2 && (toCol == fromCol+1 || toCol == fromCol-1)) ||
    (toCol == fromCol-2 && (toRow == fromRow+1 || toRow == fromRow-1)) ||
    (toCol == fromCol+2 && (toRow == fromRow+1 || toRow == fromRow-1))) && this.IsWhitePiece(toRow, toCol)){
      this.chess_Board[toRow][toCol] = piece;
      this.chess_Board[fromRow][fromCol] = '';
    }
  ...
  blackBishopMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){

  }

  blackQueenMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){

  }

  blackKingMovement(fromRow: number, fromCol: number, toRow: number, toCol: number){
